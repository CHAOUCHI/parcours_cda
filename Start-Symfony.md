...
Symfony est un Framework Web back end. Il permet de créer un serveur Web http en PHP. Le plus souvent les applications symphony, et donc les scripts PHP, sont exécutés par un serveur http. Comme apache ou NGX.
...
La philosophie de Symfony est la programmation modulaire, concrètement Symfony est composé de plusieurs modules utilisables, indépendamment. D'après la philosophie de symphonie, il n'est pas forcément nécessaire d'utiliser l'entièreté du Mork pour toute vos applications et vous pouvez sélectionner uniquement les modules que vous souhaitez utiliser et dont vous avez besoin pour créer une application légère et performante.
